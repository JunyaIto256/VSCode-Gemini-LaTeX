...
#table(
  ...
  [3], [TeX Live のインストール], [20〜40 分（ダウンロードに依存）],
...
+ 「download」をクリックし，「install-tl-windows.exe」をダウンロードして*管理者として実行*する
+ インストーラーが起動したら，「TeXworks をインストールする」の*チェックを外す*
+ 「*高度な設定*」ボタンをクリックする
+ 「選択したもの」の「*スキーム*」を「*basic スキーム（plain および latex）*」に変更する
+ スキームの直下にある「*カスタマイズ*」ボタンをクリックする
  - 「言語」タブから「*日本語*」と「*英語・米語*」を選択する
  - 「ほかのコレクション」タブから「*LaTeX 推奨パッケージ*」を選択する
  - 「*OK*」を押す
+ 右下の「*インストール*」を押して待つ（*約 1〜2 GB・20〜40 分*）

#caution-box[「高度な設定」を開かずにそのまま「インストール」を押すと，フルインストール（約 7〜8 GB・1〜2 時間）になります．必ず手順に従って「basic スキーム」＋コレクション選択を行ってください．]

インストール完了後，コマンドプロンプト（Win+R → `cmd`）で動作確認します：
...
BasicTeX（軽量版 MacTeX，約 100 MB）をインストールします：

+ `https://www.tug.org/mactex/morepackages.html` にアクセスする
+ 「BasicTeX」の `.pkg` をダウンロードしてダブルクリックでインストールする
...
インストール後，ターミナルを*再起動*してから追加パッケージを導入します：
...
#note-box[`texlive-luatex` で LuaLaTeX，`texlive-lang-japanese` で日本語サポート（jlreq クラスを含む）が導入されます．]
...
使用する処理系（LuaLaTeX / upLaTeX）をファイル先頭のコメントや
文書クラスから自動判定します．
...
#note-box[
  エンジンの選択ルール（優先順）：
  + `.tex` ファイル先頭の `% !TEX program = lualatex` などのコメント
  + `jsarticle` など旧来の日本語クラスを使っていれば upLaTeX
  + それ以外はデフォルトで LuaLaTeX
]
...
#note-box[Gemini が生成したコードがそのままコンパイルできない場合は，Agent モードであればエラーメッセージを自動で読み取って修正まで行ってくれます．Agent モードを使っていない場合は，エラーメッセージをチャットに貼り付けて修正を依頼しましょう．]
