...
+ インストーラーが起動したら「*Advanced*」ボタンをクリックする
+ 「Scheme」の項目で「*basic scheme (plain and latex)*」を選択する
+ 「Install」をクリックして待つ（*約 500 MB・10〜20 分*）

#caution-box[「Scheme」を変更せずにそのまま「Install」を押すと，フルインストール（約 7〜8 GB・1〜2 時間）になります．必ず「Advanced」から「basic scheme」を選択してください．]

インストール後，コマンドプロンプト（Win+R → `cmd`）を開き，追加パッケージを導入します：

```
tlmgr install collection-langjapanese collection-latexrecommended collection-fontsrecommended
```

#note-box[上記コマンドで日本語（`collection-langjapanese`），LaTeX 推奨パッケージ（`collection-latexrecommended`），推奨フォント（`collection-fontsrecommended`）が追加されます．追加で約 10〜20 分かかります．]
...
winget でインストールした場合は，フルインストールになります．
インストール後に不要なコレクションを削除する手間がかかるため，
公式インストーラーの「basic scheme」選択を推奨します．
